...
;; -----------------------------------------------------------------
;; In JavaScript:   {a: 1} === {a: 1}   => false  (compares places)
;; In Java:         new int[]{1,2}.equals(new int[]{1,2}) => false
;; Clojure compares VALUES, not memory addresses.
...
(comment
  ...
  ;; Equality even reaches across collection types —
  ;; it's about the value, not the container class:
  (= '(1 2 3) [1 2 3])
  ;; => true
...
(comment
  ...
  config
  ;; => {:retries 3, :timeout 5000, ...}   <- unchanged. Always.

  ;; Nested updates, still non-destructive:
  (assoc-in config [:endpoints :metrics] "https://metrics.internal")
  (update config :retries inc)

  ;; The slide-2 bug is now impossible to write:
  ;; an "audit log" of snapshots can never change after the fact.
  (def audit-log
    [config
     (assoc config :timeout 60000)
     (assoc config :retries 5)])

  (map :timeout audit-log)
  ;; => (5000 60000 5000)  -- history is free
  )

;; -----------------------------------------------------------------
;; 3. "Isn't copying everything slow?" — structural sharing
;; -----------------------------------------------------------------
;; Persistent vectors are wide trees (32-way tries).
;; assoc copies one PATH (~log32 n nodes), and shares the rest.

(comment

  (def big (vec (range 1000000)))

  ;; A full copy of a million elements, for contrast:
  (time (mapv identity big))
  ;; ~30-60 ms on a laptop

  ;; A persistent "update" of the same million elements:
  (time (assoc big 500000 :changed))
  ;; ~0.01 ms — it copied ~4 small nodes and shared the rest

  ;; And of course:
  (def big2 (assoc big 500000 :changed))
  (nth big  500000)   ;; => 500000
  (nth big2 500000)   ;; => :changed
  )
