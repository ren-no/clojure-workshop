<template>
  <svg class="clj-logo" viewBox="0 0 256 256" role="img" aria-label="Clojure logo">
    <path
      fill="#ffffff"
    ...
    />
    <!-- Grouped as lambda vs. swirls (not green vs. blue): the ring/lambda
         boundary is circular, so the groups can counter-rotate without the
         gaps between adjacent paths ever pinching -->
    <g class="clj-lambda">
      <path
        fill="#91dc47"
...
/* Different periods so the relative phase keeps evolving —
   the loop never reads as a loop */
.clj-swirls {
  animation-name: clj-sway-swirls;
  animation-duration: 11s;
}
.clj-lambda {
  animation-name: clj-sway-lambda;
  animation-duration: 8s;
}

@keyframes clj-sway-swirls {
  0%, 100% { transform: rotate(0deg); }
  25%      { transform: rotate(11deg); }
  75%      { transform: rotate(-11deg); }
}
@keyframes clj-sway-lambda {
  0%, 100% { transform: rotate(0deg); }
